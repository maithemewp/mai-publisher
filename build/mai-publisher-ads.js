  ...
  */
  ...
  if (!maiPubAdsVars.loadDelay) {
    ...
    if ('loading' === document.readyState) {
      ...
      document.addEventListener('DOMContentLoaded', maiPubRun, {
        once: true
      });
    } else {
...
function maiPubRequestSlots(slots) {
  ...
  const requestManager = {
    ...
    prebidBidsReceived: !maiPubAdsVars.magnite,
    // If Magnite is disabled, consider bids received.
  ...
  if (maiPubAdsVars.amazonUAM) {
    ...
    const uadSlots = slots.filter(slot => {
...
async function maiPubGeneratePpid(identifier = '') {
  ...
  try {
    ...
    if (!input) {
      ...
      input = typeof crypto.randomUUID === 'function' ? crypto.randomUUID() // e.g., '123e4567-e89b-12d3-a456-426614174000'.
      : Array.from(crypto.getRandomValues(new Uint8Array(16))).map(b => b.toString(16).padStart(2, '0')).join(''); // Fallback: 32-char random hex string.
    }
