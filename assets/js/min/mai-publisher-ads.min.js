...
var fallbackTimeout = 3500; // Set global failsafe timeout ~500ms after DM UI bidder timeout.
...
function maiPubDisplaySlots(slots) {
      ...
      pbjs.rp.requestBids({
        ...
        callback: function callback() {
          ...
          if (requestManager.apsBidsReceived) {
            sendAdserverRequest();
            maiPubLog('refresh() with prebid: ' + uadSlots.map(function (slot) {
  ...
  if (maiPubAdsVars.amazonUAM) {
    ...
    var _uadSlots = slots.filter(function (slot) {
    ...
    if (_uadSlots.length) {
      ...
      apstag.fetchBids(amazonConfig, function (bids) {
        ...
        if (requestManager.dmBidsReceived) {
          sendAdserverRequest();
          maiPubLog('refresh() with amazon fetch: ' + _uadSlots.map(function (slot) {
            return slot.slotID.replace('mai-ad-', '');
          }).join(', '), _uadSlots);
        }
  ...
  setTimeout(function () {
    ...
    if (!requestManager.adserverRequestSent) {
      maiPubLog('refresh() with failsafe timeout: ' + slots.map(function (slot) {
        return slot.getSlotElementId();
      }).join(', '), slots);
    }
